...
export default async function PromptPage({ params }: PageProps) {
  ...
  // Ideally, we'd also check if the user has access to the workspace.
  ...
  const transformedPrompt = {
    ...
    runs: promptData.runs.map((run) => ({
      ...
      status: run.error ? "error" : "success" as const,
  ...
  if (transformedPrompt.runs.length > 0) {
    ...
    if (sentiments.length > 0) {
      transformedPrompt.stats.avgSentiment = sentiments.reduce((a, b) => a + b, 0) / sentiments.length;
    }
  }
