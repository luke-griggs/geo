...
// Domain record type from the auth check
type DomainRecord = {
  id: string;
  name: string | null;
  domain: string;
...
// Visibility data fetcher - uses pre-verified domain record to avoid race conditions
async function getVisibilityData(
  ...
  const mentionedRunsFromAnalysis = mentions.filter((m) => m.mentioned).length;
  ...
  const userDomainKey = domainRecord.domain.toLowerCase().replace(/^www\./, "");
...
// GET /api/organizations/:organizationId/domains/:domainId/visibility
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Fetch data using the already-verified domain record (avoids race condition)
